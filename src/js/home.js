...
fetch(genres_list_http + new URLSearchParams({
    ...
    .then(data => {
        data.genres.forEach(item => {
            fetchMoviesByGenres(item.id, item.name); // Ajuste na função
        });
    })
    .catch(err => console.error(err)); // Adicionado um catch para tratar erros
...
const fetchMoviesByGenres = (id, genre) => {
    fetch(movie_genres_http + new URLSearchParams({
        ...
        .then(res => res.json())
        .then(data => {
            makeCategoryElements(`${genre}_movies`, data.results); // Ajuste na variável "genre"
        })
